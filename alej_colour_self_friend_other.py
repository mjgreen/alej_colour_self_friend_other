...
expName = 'alej_colour_self_friend_other'  # from the Builder filename that created this script
expVersion = ''
...
expInfo = {
    ...
    'blocks': ['colour then identity','identity then colour'],
    'group': ['1','2','3','4','5','6'],
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Matt\\gits\\alej_colour_self_friend_other\\alej_colour_self_friend_other.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('key_hover') is None:
        # initialise key_hover
        key_hover = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='key_hover',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    import random
    if expInfo['group']:
        mapping_number = int(expInfo['group'])
    else:
        mapping_number = random.randint(1,6)
    instruct_color_condition = "oops"
    instruct_identity_condition = "oops"
    match mapping_number:
        case 1:
            redself    = "j"
            bluefriend = "k"
            greenother = "l"
            instruct_color_condition = \
            "In the following task, you will need to report the color of the word." + \
            ...
            instruct_identity_condition = \
            "In the following task, you will need to report the identity of the word." + \
        ...
        case _:
            print("oops")
    thisExp.addData("redself", redself)
    thisExp.addData("bluefriend", bluefriend)
    thisExp.addData("greenother", greenother)
    
    # Run 'Begin Experiment' code from pull_fonts_in
    #font_files = ['font_ttfs/Arial.ttf', 'font_ttfs/verdana.ttf', 'font_ttfs/tahoma.ttf', 'font_ttfs/RAVIE.ttf', 'font_ttfs/JOKERMAN.ttf']
    font_main1 = 'Arial'
    font_main2 = 'verdana'
    font_main3 = 'tahoma'
    font_main4 = 'RAVIE'
    ...
    # --- Initialize components for Routine "increment_condition" ---
    
    # --- Initialize components for Routine "handover" ---
    text_hover = visual.TextStim(win=win, name='text_hover',
        text='',
        font='Arial',
        ...
        languageStyle='LTR',
        depth=-1.0);
    key_hover = keyboard.Keyboard(deviceName='key_hover')
    ...
    # --- Initialize components for Routine "assign_correctkey" ---
    ...
    put_feedback = visual.TextStim(win=win, name='put_feedback',
        text='',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='black', colorSpace='rgb', opacity=None, 
    ...
    order_of_conditions=[]
    print("block order is: ", expInfo['blocks'])
    if expInfo['blocks'] == "colour then identity":
        order_of_conditions=['trials_color.xlsx','trials_identity.xlsx', 'trials_color.xlsx','trials_identity.xlsx']
    if expInfo['blocks'] == "identity then colour":
        order_of_conditions=['trials_identity.xlsx','trials_color.xlsx', 'trials_identity.xlsx','trials_color.xlsx']
    print(order_of_conditions)
    condition_number = -1
    ...
    for thisCondition in conditions:
        ...
        # --- Prepare to start Routine "increment_condition" ---
        # create an object to store info about Routine increment_condition
        increment_condition = data.Routine(
            name='increment_condition',
            components=[],
        )
        increment_condition.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from increment1
        condition_number = condition_number + 1
        condition = order_of_conditions[condition_number]
        print("This condition is: " + condition)
        thisExp.addData("condition", condition)
        # store start times for increment_condition
        increment_condition.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        increment_condition.tStart = globalClock.getTime(format='float')
        increment_condition.status = STARTED
        thisExp.addData('increment_condition.started', increment_condition.tStart)
        increment_condition.maxDuration = None
        # keep track of which components have finished
        increment_conditionComponents = increment_condition.components
        for thisComponent in increment_condition.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "increment_condition" ---
        increment_condition.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            ...
            if thisExp.status == PAUSED:
                pauseExperiment(
                    ...
                    win=win, 
                    timers=[routineTimer, globalClock], 
                    currentRoutine=increment_condition,
                )
                # skip the frame we paused on
                continue
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                increment_condition.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in increment_condition.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        # --- Ending Routine "increment_condition" ---
        for thisComponent in increment_condition.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for increment_condition
        increment_condition.tStop = globalClock.getTime(format='float')
        increment_condition.tStopRefresh = tThisFlipGlobal
        thisExp.addData('increment_condition.stopped', increment_condition.tStop)
        # the Routine "increment_condition" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        
        # --- Prepare to start Routine "handover" ---
        # create an object to store info about Routine handover
        handover = data.Routine(
            name='handover',
            components=[text_hover, key_hover],
        )
        handover.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from code_hover
        inittext = "There will be some practice trials with feedback, followed by some experimental trials without feedback. Press space when you are ready to continue."
        halfwaytext = "You have finished the practice trials. Press space when you are ready for the experimental trials"
        hovertext = "oops"
        ...
        if condition_number == 2:
            hovertext = halfwaytext
        if condition_number == 3:
            continueRoutine = False
        text_hover.setText(hovertext)
        # create starting attributes for key_hover
        key_hover.keys = []
        key_hover.rt = []
        _key_hover_allKeys = []
        # store start times for handover
        handover.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        handover.tStart = globalClock.getTime(format='float')
        handover.status = STARTED
        thisExp.addData('handover.started', handover.tStart)
        handover.maxDuration = None
        # keep track of which components have finished
        handoverComponents = handover.components
        for thisComponent in handover.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "handover" ---
        handover.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # if trial has changed, end Routine now
            if hasattr(thisCondition, 'status') and thisCondition.status == STOPPING:
                continueRoutine = False
            ...
            # if text_hover is starting this frame...
            if text_hover.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                text_hover.frameNStart = frameN  # exact frame index
                text_hover.tStart = t  # local t and not account for scr refresh
                text_hover.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(text_hover, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'text_hover.started')
                # update status
                text_hover.status = STARTED
                text_hover.setAutoDraw(True)
            
            # if text_hover is active this frame...
            if text_hover.status == STARTED:
                # update params
                pass
            ...
            # if key_hover is starting this frame...
            if key_hover.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                key_hover.frameNStart = frameN  # exact frame index
                key_hover.tStart = t  # local t and not account for scr refresh
                key_hover.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(key_hover, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'key_hover.started')
                # update status
                key_hover.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(key_hover.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(key_hover.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if key_hover.status == STARTED and not waitOnFlip:
                theseKeys = key_hover.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
                _key_hover_allKeys.extend(theseKeys)
                if len(_key_hover_allKeys):
                    key_hover.keys = _key_hover_allKeys[-1].name  # just the last key pressed
                    key_hover.rt = _key_hover_allKeys[-1].rt
                    key_hover.duration = _key_hover_allKeys[-1].duration
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                handover.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in handover.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        # --- Ending Routine "handover" ---
        for thisComponent in handover.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for handover
        handover.tStop = globalClock.getTime(format='float')
        handover.tStopRefresh = tThisFlipGlobal
        thisExp.addData('handover.stopped', handover.tStop)
        # check responses
        if key_hover.keys in ['', [], None]:  # No response was made
            key_hover.keys = None
        conditions.addData('key_hover.keys',key_hover.keys)
        if key_hover.keys != None:  # we had a response
            conditions.addData('key_hover.rt', key_hover.rt)
            conditions.addData('key_hover.duration', key_hover.duration)
        # the Routine "handover" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        ...
        while continueRoutine:
            ...
            if key_resp_instructions.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
        ...
        conditions.addData('key_resp_instructions.keys',key_resp_instructions.keys)
        if key_resp_instructions.keys != None:  # we had a response
            conditions.addData('key_resp_instructions.rt', key_resp_instructions.rt)
            conditions.addData('key_resp_instructions.duration', key_resp_instructions.duration)
        ...
        for thisTrial in trials:
            ...
            # --- Prepare to start Routine "assign_correctkey" ---
            # create an object to store info about Routine assign_correctkey
            assign_correctkey = data.Routine(
                name='assign_correctkey',
                components=[],
            )
            assign_correctkey.status = NOT_STARTED
            continueRoutine = True
            # update component parameters for each repeat
            # Run 'Begin Routine' code from say_what_correctkey_is
            correctkey = "oops"
            if mapping_number == 1:
                if correctresponse == "redself":
                    correctkey = 'j'
                if correctresponse == "bluefriend":
                    correctkey = 'k'
                if correctresponse == "greenother":
                    correctkey = 'l'
            if mapping_number == 2:
                if correctresponse == "redself":
                    correctkey = 'j'
            ...
            # store start times for assign_correctkey
            assign_correctkey.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
            assign_correctkey.tStart = globalClock.getTime(format='float')
            assign_correctkey.status = STARTED
            thisExp.addData('assign_correctkey.started', assign_correctkey.tStart)
            assign_correctkey.maxDuration = None
            # keep track of which components have finished
            assign_correctkeyComponents = assign_correctkey.components
            for thisComponent in assign_correctkey.components:
                thisComponent.tStart = None
                thisComponent.tStop = None
                thisComponent.tStartRefresh = None
                thisComponent.tStopRefresh = None
            ...
            # --- Run Routine "assign_correctkey" ---
            assign_correctkey.forceEnded = routineForceEnded = not continueRoutine
            while continueRoutine:
                # if trial has changed, end Routine now
                ...
                if thisExp.status == PAUSED:
                    pauseExperiment(
                        ...
                        win=win, 
                        timers=[routineTimer, globalClock], 
                        currentRoutine=assign_correctkey,
                    )
                    # skip the frame we paused on
                    continue
                ...
                # check if all components have finished
                if not continueRoutine:  # a component has requested a forced-end of Routine
                    assign_correctkey.forceEnded = routineForceEnded = True
                    break
                continueRoutine = False  # will revert to True if at least one component still running
                for thisComponent in assign_correctkey.components:
                    if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
            ...
            # --- Ending Routine "assign_correctkey" ---
            for thisComponent in assign_correctkey.components:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            # store stop times for assign_correctkey
            assign_correctkey.tStop = globalClock.getTime(format='float')
            assign_correctkey.tStopRefresh = tThisFlipGlobal
            thisExp.addData('assign_correctkey.stopped', assign_correctkey.tStop)
            # the Routine "assign_correctkey" was not non-slip safe, so reset the non-slip timer
            routineTimer.reset()
            ...
            stimulus = data.Routine(
                name='stimulus',
                components=[fix, stimuli, response],
            )
            ...
            while continueRoutine:
                ...
                if response.status == NOT_STARTED and fix.status==FINISHED:
                    # keep track of start time/frame for later
                ...
                if response.status == STARTED and not waitOnFlip:
                    theseKeys = response.getKeys(keyList=['j', 'k', 'l'], ignoreKeys=["escape"], waitRelease=False)
            ...
            feedback = data.Routine(
                name='feedback',
                components=[put_feedback],
            )
            feedback.status = NOT_STARTED
            continueRoutine = True
            # update component parameters for each repeat
            # Run 'Begin Routine' code from assign_or_skip_feedback
            feedback_text = "oops"
            if response.corr == 1:
                feedback_text = "Correct"
            if response.corr == 0:
                feedback_text = "wrong"
            print(task, word, color, font_variable, feedback_text, "actual response was ", response.keys, "correct anwser was ", correctkey)
            if condition_number >= 2:
                continueRoutine = False
            
                    
            put_feedback.setText(feedback_text)
            ...
            while continueRoutine and routineTimer.getTime() < 1.0:
                ...
                # *put_feedback* updates
                
                # if put_feedback is starting this frame...
                if put_feedback.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    put_feedback.frameNStart = frameN  # exact frame index
                    put_feedback.tStart = t  # local t and not account for scr refresh
                    put_feedback.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(put_feedback, 'tStartRefresh')  # time at next scr refresh
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'put_feedback.started')
                    # update status
                    put_feedback.status = STARTED
                    put_feedback.setAutoDraw(True)
                
                # if put_feedback is active this frame...
                if put_feedback.status == STARTED:
                    # update params
                    pass
                
                # if put_feedback is stopping this frame...
                if put_feedback.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > put_feedback.tStartRefresh + 1.0-frameTolerance:
                        # keep track of stop time/frame for later
                        put_feedback.tStop = t  # not accounting for scr refresh
                        put_feedback.tStopRefresh = tThisFlipGlobal  # on global time
                        put_feedback.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 'put_feedback.stopped')
                        # update status
                        put_feedback.status = FINISHED
                        put_feedback.setAutoDraw(False)
        ...
        if conditions.status == PAUSED:
            ...
            # once done pausing, restore running status
            conditions.status = STARTED
    # completed 4.0 repeats of 'conditions'
