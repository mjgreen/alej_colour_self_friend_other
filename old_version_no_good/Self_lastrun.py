...
This experiment was created using PsychoPy3 Experiment Builder (v2025.1.1),
    on Tue 30 Sep 2025 11:30:03 AM BST
...
expName = 'Self'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/home/matt/Downloads/self_task/Self_lastrun.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('goodbye_response') is None:
        # initialise goodbye_response
        goodbye_response = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='goodbye_response',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    goodbye_text = visual.TextStim(win=win, name='goodbye_text',
        ...
        languageStyle='LTR',
        depth=0.0);
    goodbye_response = keyboard.Keyboard(deviceName='goodbye_response')
    ...
    for thisBlock in blocks:
        ...
        trials = data.TrialHandler2(
            ...
            extraInfo=expInfo, 
            originPath=-1, 
            trialList=data.importConditions(trial_list), 
            seed=None, 
        )
    ...
    goodbye = data.Routine(
        name='goodbye',
        components=[goodbye_text, goodbye_response],
    )
    goodbye.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # create starting attributes for goodbye_response
    ...
    while continueRoutine:
        ...
        if goodbye_text.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > goodbye_text.tStartRefresh + 10-frameTolerance:
                # keep track of stop time/frame for later
        ...
        # if goodbye_response is starting this frame...
        if goodbye_response.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'goodbye_response.started')
            # update status
        ...
        if goodbye_response.status == STARTED and not waitOnFlip:
            theseKeys = goodbye_response.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            ...
            if len(_goodbye_response_allKeys):
                ...
                goodbye_response.duration = _goodbye_response_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
    ...
    if goodbye_response.keys != None:  # we had a response
        ...
        thisExp.addData('goodbye_response.duration', goodbye_response.duration)
    thisExp.nextEntry()
